...
const verifyCalendarState = (token) =>
  jwt.verify(token, calendarStateSecret);

// Google OAuth - use passport's built-in callback
router.get("/google", (req, res, next) => {
  if (!process.env.CLIENT_ID || !process.env.CLIENT_SECRET) {
    return res.redirect(
      `${FRONTEND_URL}/auth/login?error=google_not_configured`,
    );
  }
...
router.get("/google/callback", (req, res, next) => {
  passport.authenticate("google", { session: false }, async (err, user) => {
    if (err) {
      ...
      return res.redirect(
        `${FRONTEND_URL}/auth/login?error=google_callback_error`,
      );
    }

    if (!user) {
      return res.redirect(
        `${FRONTEND_URL}/auth/login?error=google_auth_failed`,
      );
    }
...
router.get("/google-calendar", protect, (req, res) => {
  if (!calendarOAuthClient) {
    return res.redirect(
      `${FRONTEND_URL}/dashboard/student?error=google_not_configured`,
    );
  }
...
router.get("/google-calendar/callback", async (req, res) => {
  ...
  if (!code || !state) {
    return res.redirect(
      `${FRONTEND_URL}/dashboard/student?error=google_calendar_missing_code`,
    );
  }
  ...
  try {
    ...
    if (!user) {
      return res.redirect(
        `${FRONTEND_URL}/dashboard/student?error=google_calendar_user_missing`,
      );
    }
    ...
    if (!refreshToken) {
      return res.redirect(
        `${FRONTEND_URL}/dashboard/student?error=google_calendar_no_refresh_token`,
      );
    }
    ...
    return res.redirect(
      `${FRONTEND_URL}/dashboard/student?calendar=connected`,
    );
  } catch (error) {
    console.error("Google Calendar connect error:", error);
    return res.redirect(
      `${FRONTEND_URL}/dashboard/student?error=google_calendar_callback_error`,
    );
  }
});
...
router.patch(
  "/users/:id/convert-student",
  authController.convertAdminToStudent,
);
router.patch(
  "/users/:id/transfer-super-admin",
