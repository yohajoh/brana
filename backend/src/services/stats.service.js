...
export const getOverviewStats = async () => {
  ...
  const [
    ...
    prisma.rental.count({ where: /** @type {any} */ ({ status: 'COMPLETED' }) }),
    // Revenue this month
...
export const getBooksStats = async () => {
  const [
    ...
    prisma.book.findMany({
      ...
      select: {
        id: true, title: true, available: true, copies: true,
  ...
  const physicalBooksDetails = await prisma.book.findMany({
    ...
    select: { id: true, title: true, cover_image_url: true, author: { select: { name: true } } },
  });

  const physicalMap = Object.fromEntries(physicalBooksDetails.map((b) => [b.id, b]));
  ...
  const digitalMap = Object.fromEntries(digitalBooksDetails.map((b) => [b.id, b]));
  ...
  reviewDistribution.forEach((r) => { ratingDist[r.rating] = r._count.rating; });
...
export const getUserStats = async () => {
  ...
  const [
    ...
    prisma.user.findMany({
      ...
      select: { id: true, name: true, email: true, student_id: true, created_at: true, is_confirmed: true },
  ...
  const confirmedCount =
    confirmedVsUnconfirmed.find((r) => r.is_confirmed)?.['_count']?.['is_confirmed'] ?? 0;
...
export const getRentalStats = async () => {
  ...
  const completionRate =
    total > 0
      ? parseFloat(((((statusMap['COMPLETED'] ?? 0) + (statusMap['RETURNED'] ?? 0)) / total) * 100).toFixed(1))
      : 0;
