    ...
    <div className="p-6 lg:p-12 space-y-8 max-w-[1600px] mx-auto">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-[#AE9E85] font-medium mt-2">{t("admin_activity_logs.subtitle")}</p>
        </div>
        ...
        <div className="divide-y divide-[#E1D2BD]/30">
          {isLoading ? (
            <div className="py-24 text-center">
              ...
              <p className="text-[#AE9E85] font-medium italic">{t("admin_activity_logs.loading")}</p>
            </div>
          ) : logs.length === 0 ? (
            <div className="py-24 text-center text-[#AE9E85] font-medium italic">{t("admin_activity_logs.no_logs")}</div>
          ) : (
            logs.map((row) => (
              <div key={row.id} className="grid grid-cols-[1.5fr_1fr_3fr_1fr] gap-4 items-center px-8 py-5 hover:bg-[#FDFAF6]/50 transition-colors group">
                <div className="min-w-0">
                  <p className="text-sm font-bold text-[#2B1A10] truncate">{row.admin?.name || t("admin_activity_logs.system")}</p>
