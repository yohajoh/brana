...
import { Bell, ChevronRight, CheckCheck, AlertCircle, BookOpen, Clock, Info } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { useNotifications, useMarkAsRead, useMarkAllAsRead, type Notification } from "@/lib/hooks/useNotifications";
...
const typeConfig = (type: string) => {
  switch (type) {
    case "ALERT":
    case "OVERDUE":  return { icon: AlertCircle, dot: "bg-red-500",     bg: "bg-red-50",     text: "text-red-600",   border: "border-red-100" };
    case "REMINDER": return { icon: Clock,        dot: "bg-amber-400",   bg: "bg-amber-50",   text: "text-amber-600", border: "border-amber-100" };
    case "NEW_BOOK": return { icon: BookOpen,      dot: "bg-emerald-500", bg: "bg-emerald-50", text: "text-emerald-600", border: "border-emerald-100" };
    default:         return { icon: Info,          dot: "bg-[#142b6f]",   bg: "bg-blue-50",    text: "text-blue-600",  border: "border-blue-100" };
  }
};
...
function NotificationDropdownContent() {
  ...
  const reposition = useCallback(() => {
    ...
    if (rect.right - PANEL_W < 8) rightEdge = window.innerWidth - PANEL_W - 8;
    setPanelPos({ top: rect.bottom + 6, right: Math.max(8, rightEdge) });
  }, []);

  const toggle = () => {
    if (!isOpen) { reposition(); refetch(); }
    setIsOpen(v => !v);
  };
  ...
  const handleClick = (n: Notification) => {
    setIsOpen(false);
    router.push(`/dashboard/student/notifications?notification=${n.id}`);
    if (!n.is_read) markOne.mutate(n.id, { onSuccess: () => refetch() });
  };

  const handleViewAll = () => { setIsOpen(false); router.push("/dashboard/student/notifications"); };

  const handleMarkAll = async () => {
    try { await markAll.mutateAsync(); refetch(); }
    catch { /* silent */ }
  };
    ...
    <>
      ...
      <div ref={wrapperRef} className="relative inline-flex">
        <button
          ...
          type="button"
          onClick={toggle}
          aria-label={String(t("student_notifications.title"))}
        ...
        {/* Badge — on wrapper div, never clipped */}
        <AnimatePresence>
          {unreadCount > 0 && (
            <motion.span
              ...
              style={{ top: "-6px", right: "-6px", minWidth: "18px", height: "18px", padding: "0 4px", fontSize: "9px", border: "2.5px solid white", zIndex: 10, boxShadow: "0 2px 6px rgba(239,68,68,0.5)" }}
            >
          ...
          {isOpen && (
            <motion.div
              ...
              style={{
                ...
                width: Math.min(380, (typeof window !== "undefined" ? window.innerWidth : 400) - 16),
                ...
                <div className="flex items-center gap-1">
                  {unreadCount > 0 && (
                    <button onClick={handleMarkAll} disabled={markAll.isPending}
                      className="flex items-center gap-1 text-[10px] font-bold text-[#142b6f] hover:text-[#0d0d0d] px-2 py-1 rounded-lg hover:bg-[#f5f4f0] transition-colors disabled:opacity-50">
                      <CheckCheck size={11} /> All read
                    </button>
                  )}
                  <button onClick={handleViewAll}
                    className="flex items-center gap-1 text-[11px] font-bold text-[#142b6f] hover:text-[#0d0d0d] px-2.5 py-1.5 rounded-lg hover:bg-[#f5f4f0] transition-colors">
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p className={`text-[12.5px] leading-snug line-clamp-2 group-hover:line-clamp-none ${!n.is_read ? "font-semibold text-[#0d0d0d]" : "font-medium text-[#0d0d0d]/65"}`}>
                              {n.message}
                            </p>
                          ...
                          {!n.is_read && <span className={`w-2 h-2 rounded-full shrink-0 mt-2 ${cfg.dot}`} />}
                        </motion.button>
                      );
                    })}
                  </div>
...
export function NotificationDropdown() {
  return (
    <Suspense fallback={
      <button className="relative flex items-center justify-center w-9 h-9 rounded-xl border border-[#e2e0e7] bg-white text-[#374151]">
        <Bell size={16} strokeWidth={1.75} />
      </button>
    }>
